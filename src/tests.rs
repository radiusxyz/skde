...
mod tests {
    ...
    #[test]
    fn test_encryption_decryption_stress() {
        const NUM_ITERATIONS: usize = 10;
        let test_message = "SKDE encryption-decryption consistency check.";
        let mut success_count = 0;
        let mut failure_logs = Vec::new();
        ...
        for i in 0..NUM_ITERATIONS {
            ...
            // Generate partial keys with proof
            let partial_keys_result: Result<Vec<_>, _> = (0..MAX_SEQUENCER_NUMBER)
                .map(|_| {
                    let (secret, partial) = generate_partial_key(&skde_params)?;
                    let proof = prove_partial_key_validity(&skde_params, &secret)?;
            ...
            match partial_keys_result {
                Ok(partial_keys) => {
                    let aggregated_key = aggregate_key(&skde_params, &partial_keys);
                    let encryption_key = aggregated_key.u.clone();
                    let secret_key = solve_time_lock_puzzle(&skde_params, &aggregated_key);

                    match secret_key {
                        Ok(secret_key) => {
                            for hybrid in [false, true] {
                                let label = if hybrid { "Hybrid" } else { "Standard" };

                                // Encrypt
                                let ciphertext_result =
                                    encrypt(&skde_params, test_message, &encryption_key, hybrid);
                                if let Err(err) = ciphertext_result {
                                    failure_logs.push(format!(
                                        "Iteration {} [{}]: encryption failed: {:?}",
                                        i + 1,
                                        label,
                                        err
                                    ));
                                ...
                                match decrypt(&skde_params, &ciphertext, &secret_key.sk) {
                                    Ok(decrypted) => {
                                        if decrypted == test_message {
                                            success_count += 1;
                                        } else {
                                            failure_logs.push(format!(
                                                "Iteration {} [{}]: decrypted message mismatch",
                                                i + 1,
                                                label
                                            ));
                                        }
                        ...
                        Err(err) => {
                            failure_logs.push(format!(
                                "Iteration {}: puzzle solving failed: {:?}",
                                i + 1,
                                err
                            ));
                        }
                ...
                Err(err) => {
                    failure_logs.push(format!(
                        "Iteration {}: key/proof generation failed: {:?}",
                        i + 1,
                        err
                    ));
                }
        ...
        println!("\n==== SKDE Encryption-Decryption Stress Test ====");
        println!("✅ Successful validations: {}", success_count);
        println!("❌ Failed validations:     {}", failure_logs.len());
        ...
        assert!(
            failure_logs.is_empty(),
            "Encryption-decryption mismatch or error occurred in some iterations"
        );
    }
}
